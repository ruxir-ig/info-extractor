...
vague feilds can also have only comments
...
def extract_with_gemini(file_bytes: bytes, api_key: str) -> str:
...
def parse_json_to_dataframe(json_result: str) -> pd.DataFrame:
    data = json.loads(json_result)

    # Sometimes the AI wraps the result in an object
    ...
    desired_cols = ["Key", "Value", "Comments"]
    ...
    if uploaded_file and st.button("Start Extraction"):
        with st.spinner("Analyzing document..."):
            try:
                ...
                json_result = extract_with_gemini(file_bytes, api_key)
